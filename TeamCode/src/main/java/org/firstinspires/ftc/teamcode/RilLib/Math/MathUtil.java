...
public final class MathUtil {
     ...
     * @param maxValue The maximum magnitude of the input. Can be infinite.
     * @return The value after the deadband is applied.
     */
    public static double applyDeadband(double value, double deadband, double maxValue) {
        if (Math.abs(value) > deadband) {
            if (maxValue / deadband > 1.0e12) {
            ...
            return maxValue * (value - Math.copySign(deadband, value)) / (maxValue - deadband);
        } else {
            return 0.0;
        }
    }
